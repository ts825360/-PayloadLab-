# 웹 보안 6대 취약점 조사 (공격 유형 조사)

> 이 문서는 아래 두 조사 문서의 겹치는 내용을 하나로 합치고, 중복을 제거해 정리한 것입니다.
> - `web_hacking_lens_learning.md`
> - `PayloadLab_WebSecurity_6_Research.md`
>
> Payload 예시·통계 근거는 `Payload-Dataset-Easy-Medium.md`(491건)를 참고했습니다.
...
Dreamhack의 웹 해킹 학습 구성을 참고하되, 초보자가 이해하기 어려운 고급 우회 기법은 제외하고 **EASY / MEDIUM** 두 단계로 조사한다.

- **EASY**: 취약점이 왜 발생하는지 기본 원리를 이해하는 단계
- **MEDIUM**: EASY에서 배운 원리를 이용해 조금 더 복잡한 상황(우회·추론·구조 분석)을 이해하는 단계
...
초보자는 웹사이트를 단순히 `입력 → 결과`로 생각하지만, 실제로는 `사용자 → 웹 서버 → SQL Query/명령/렌더링 → 처리 엔진 → 결과 → 웹페이지`라는 중간 과정이 있다. 이 중간 과정을 모르기 때문에 공격 문자열을 입력해도 왜 결과가 바뀌는지 이해하기 어렵다. Lens는 이 중간 과정을 유형별로 눈에 보이게 풀어준다.
...
- **초보자가 막히는 부분**
  ...
  5. Payload를 외워서 입력했는데 왜 결과가 달라지는지, 실제 SQL Query가 어떻게 바뀌는지 모름
- **Lens에서 보여줄 것**
...
- **초보자가 막히는 부분**
  ...
  5. 데이터베이스 구조 및 DBMS별 문법 차이(MySQL, SQLite 등)를 어느 정도 알아야 함
...
### 3.6 Command Injection / Linux Shell
...
`Payload-Dataset-Easy-Medium.md`에 실제 Payload마다 출처가 기록되어 있습니다. 유형별 대표 출처는 다음과 같습니다.

| 취약점 | 출처 |
|---|---|
| SQL Injection | [PayloadsAllTheThings - SQL Injection](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/SQL%20Injection/README.md), [PayloadsAllTheThings - SQLite Injection](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/SQL%20Injection/SQLite%20Injection.md) |
| XSS | [PayloadsAllTheThings - XSS Injection](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/XSS%20Injection/README.md), [OWASP XSS Filter Evasion Cheat Sheet](https://owasp.org/www-community/xss-filter-evasion-cheatsheet) |
| IDOR | 별도 공개 출처 없음 — PayloadLab 자체 제작 시나리오 (Dataset에 Source URL 미기재) |
| JWT / Authentication | [PayloadsAllTheThings - JSON Web Token](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/JSON%20Web%20Token/README.md), [OWASP JWT Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html), [PortSwigger - JWT Attacks](https://portswigger.net/web-security/jwt), [RFC 7519](https://www.rfc-editor.org/rfc/rfc7519) |
| SSRF | [PayloadsAllTheThings - Server Side Request Forgery](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Server%20Side%20Request%20Forgery/README.md) |
| Command Injection | [PayloadsAllTheThings - Command Injection](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Command%20Injection/README.md) |

> IDOR은 Dataset 전체(63건)에 공개 출처 URL이 없고 "PayloadLab (자체 제작 시나리오)"로만 표기되어 있습니다. 필요하면 OWASP Access Control 관련 자료를 별도로 찾아 보강하는 것을 권장합니다.
